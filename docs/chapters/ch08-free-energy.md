...
正如第 8.1 节所解释的，自由能计算与计算可观测量的玻尔兹曼加权平均的模拟在本质上是不同的：
...
一个物种的化学势的完整表达式依赖于普朗克常数，通过热 de Broglie 波长和可能的分子内部配分函数。然而，正如我们在第 8.1 节中所论证的，经典模拟的结果不依赖于普朗克常数。
...
???+ example "例 18（化学势：重叠分布法）"
    ...
    在图 8.4 中，我们展示了如何从 $f_0(\Delta U)$ 和 $f_1(\Delta U)$ 关于 $\Delta U$ 的图中通过式（8.6.8）获得 $\mu^{\text{ex}}(\Delta U)$。左图的结果对应于 $\rho = 0.7$ 的 Lennard-Jones 流体，右图的结果对应于 $\rho = 1.0$。
...
当然，像上面那样由 $\delta$ 函数之和构成的分布并不适合作图表示。事实上，它看起来一点也不像我们习惯的直方图。为看清这一点，考虑在一次无偏倚模拟中生成的分布函数。把 $V_k = 0$ 代入最大似然条件，可以看到对所有 $n$ 都有 $p_n = 1$。那么，一个由等幅 $\delta$ 函数之和构成的分布函数，怎么会代表玻尔兹曼分布呢？

答案是：在玻尔兹曼权重高的区域，$\delta$ 函数的数目远多于权重低的区域。因此，如果我们把上述分布函数在宽度为 $\Delta$ 的箱上积分来构造直方图，就会重新得到一个看起来确实像玻尔兹曼分布的直方图。重要的是，用 MBAR 我们只需一次性确定最优分布，之后再做分箱或曲线拟合即可。

直方图重连方法最常见的应用，是计算某个常见热力学变量（如数密度 $\rho$ 或能量密度 $e$）的分布随某个强度热力学变量（如逆温 $\beta$ 或逸度 $z$）的变化。在这些情形下，我们可以用上述两式，在不同于参考态 $\beta_0$ 与 $z_0$ 的 $\beta$ 或 $z$ 值处重建所需的直方图，此时偏倚势 $V_k$ 具有 $(\beta_k-\beta_0)U(X)$ 或 $(\ln z - \ln z_0)N$ 的形式。在例 19 中，我们将展示这种“直方图重加权”方法的一个应用。
...
???+ example "例证 11（外场中的理想气体分子）"
    ...
    *图 8.6　在位置 $z$ 处找到理想气体粒子的概率。左图给出各个窗口的结果，右图给出由自洽直方图方法重建所得的分布函数。*
    ...
    对每个窗口，我们做 $M$ 次采样来估计在位置 $z$ 处找到理想气体粒子的概率 $p_i(z)$。这类模拟的结果示于图 8.6（左）。诸如 MBAR 这样的自洽直方图方法可以用来重建所需的分布 $p_0(z)$，计算结果示于图 8.6（右）。
...
这就是式 (8.6.10)。Crooks [[389]](references.md#ref-389) 给出了不局限于哈密顿体系的、更一般的推导。特别地，Crooks 证明了只要体系的动力学是马尔可夫的且微观可逆，上述结果仍然成立。Crooks 的结果意味着，即使体系的“时间演化”由 Metropolis Monte Carlo 方案决定，该结果依然有效。更多细节见附录 E。

Jarzynski 的结果及 Crooks 的推广既令人惊讶又优雅，具有重大的概念意义。然而，几乎没有证据表明计算自由能差的非平衡方法优于现有的平衡方法[[392]](references.md#ref-392)。

例外似乎是玻璃态体系中自由能差的计算[[393]](references.md#ref-393)——在那里热力学积分及相关方法干脆失效。然而，当其他自由能方法也可以使用时，它们至少与 Jarzynski 方法一样好。限制 Jarzynski 方法实际应用的根本原因在于：对于远离平衡的变换，上述关系式的统计精度可能相当差。我们对瞬时切换的极限早已知道这一点。就像 Widom 方法的粒子移除版本一样，我们希望计算的平均值的主要贡献来自很少被采样到的初始构型。这正是在测量化学势时，“粒子移除”法无法成为“粒子插入”方案之可行替代的原因。为了在非平衡自由能计算的背景下说明这一问题，我们考虑一种不改变体系自由能的哈密顿量变化。
