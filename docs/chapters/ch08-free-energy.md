...
为了阐明这一根本问题的本质，回顾第3章中图 3.1是有益的，在那里我们通过在尼罗河中进行随机行走来测量其平均深度，作为重要性采样的一个简单示例。需要注意的关键点是，这种随机行走并不是采样尼罗河表面积的好策略，而尼罗河的表面积等价于相空间中的体积。如果我们想通过比较在两条河流中分别停留的相对时间来确定两条河流面积之比，随机行走也同样无用。
...
???+ example "例证 8（分子动力学与 Monte Carlo 的对比）"
    ...
    图 8.1给出了一种纳米多孔晶体材料的示意图，它由若干空腔经狭窄的“窗口”连通而成。在普通的 MD/MC 模拟中，窗口几乎采样不到。图 8.1(b)正说明了这一采样困难：它显示了分子在该孔中的典型MD轨迹——大部分时间分子被困在某个空腔中，在图 8.1(b)所示的例子中，分子在模拟期间仅跳跃了一次到相邻空腔。对于较大的分子，在窗口区域找到分子的概率可能变得太小而无法用暴力模拟来采样。

    自由能计算提供了计算在窗口中找到分子的概率的可能性，而无需等待该事件自发发生。图 8.1所示的系统足够简单，我们可以解析地计算单个原子的自由能势垒。然而，如果孔中有更多原子或更复杂的分子，我们就需要数值自由能计算来计算在窗口中找到粒子的概率。
...
#### 无界面的共存
除了上述在同一个模拟盒中模拟两个（或多个）相共存的方法外，还有几种无需创建界面即可研究相共存的方案。在流体中，最著名的方法是Panagiotopoulos的Gibbs 系综方法^[211,213,214,220]，这在第6.6节中讨论。对于下文内容，重要的是指出Gibbs 系综方法的主要局限性：其平衡依赖于两个周期性重复系统之间的体积和粒子交换。然而，如果两个相中至少有一个是致密的（例如在典型液体密度下），粒子交换的接受率变得可以忽略不计，Gibbs 系综方法将失败。当共存相之一是固体时，这个问题实际上是无法克服的。%
[^6]
...
正如第8.2节所解释的，我们的目标不是提供所有相关自由能算法的综述。
...
正如第8.1节所解释的，自由能计算与计算可观测量的Boltzmann加权平均的模拟在本质上是不同的：
...
例如硬球^[318,319,320,321,322]或Lennard-Jones粒子^[73,83,323]的流体或固体。
...
这种计算在生物分子建模中具有特别重要的意义（参见例如^[324,325]）。
...
在第8.7节中，我们将讨论即使系统的哈密顿量以有限速率变化时，
如何仍然可以计算自由能差。
...
1. 该算法展示了恒定$N$、$V$和$T$系统的Widom试探粒子方法的基本结构。
   此函数通常在Monte Carlo模拟的采样步骤中调用，例如算法 1中的函数`sample`。
...
在算法 18中，我们展示了如何在模拟中实现粒子插入方法。
不出所料，鉴于Coulomb势的长程性，插入离子时需要特别小心。
人们可能认为电中性要求联合插入正离子和负离子，使整体呈中性。
然而，这是不必要的，只要离子总是与其均匀中和背景一起插入^[329]。

**与逸度的关系**

一个物种的化学势的完整表达式依赖于Planck常数，通过热de Broglie波长和可能的分子内部配分函数。然而，正如我们在第8.1节中所论证的，经典模拟的结果不依赖于Planck常数。
...
公式(8.5.5)是针对空间均匀系统推导的。Widom^[330]也考虑了空间非均匀系统的情况。
在这种情况下，$\mu^{\mathrm{ex}}$显式依赖于位置$\mathbf{r}$。
然而，在平衡状态下，化学势本身在整个系统中是常数（但参见文献^[139]中的公式4）。
换句话说，
...
???+ example "例 17（化学势：粒子插入法）"
    ...
    在本例中，我们使用Widom试探粒子法来确定Lennard-Jones流体的超额化学势。所用的算法是恒定 $N$、$V$、$T$ 下进行Monte Carlo模拟的基本算法（算法 1和算法 2）与确定超额化学势（算法 18）的组合。
    ...
    在案例研究9中，我们进行了巨正则Monte Carlo模拟来确定Lennard-Jones流体的状态方程。在巨正则系综中，体积、化学势和温度是给定的；密度在模拟过程中确定。当然，我们也可以在模拟过程中使用Widom方法计算化学势。图 8.2展示了给定化学势与测量化学势的比较。

    ![图 8.2](../images/fig_8_2.png)

    *图 8.2　Lennard-Jones流体的超额化学势（$T = 2.0$），分别由状态方程、巨正则Monte Carlo和试探粒子插入法计算得到。*
...
???+ example "例 18（化学势：重叠分布法）"
    ...
    在案例研究15中，我们使用Widom试探粒子法确定Lennard-Jones流体的化学势。该方法在高密度下会失效，因为在这些密度下，将粒子插入到使式（8.5.5）中 $\exp(-\beta \Delta U)$ 不可忽略的位置变得极其不可能。然而正是那些不太可能的插入主导了平均值 $\langle \exp(-\beta \Delta U) \rangle$。由于有利的插入事件极为稀少，此类事件的数量受到较大的统计涨落影响，因此我们对 $\langle \exp(-\beta \Delta U) \rangle$ 的估计噪声较大。重叠分布法并没有消除这个问题，但它提供了一种很好的诊断工具来检测此类采样问题。

    为了实现重叠分布法，我们需要进行两次模拟：一次使用 $N+1$ 个粒子的系统（系统1），另一次使用 $N$ 个粒子和一个理想气体粒子的系统（系统0）。对于这两个系统，我们确定能量差的分布，即式（8.6.3）和（8.6.4）。对于系统1，能量差 $\Delta U$ 定义为：如果随机选择一个粒子并将其转化为理想气体粒子，系统总能量的变化量。现在我们对该系统中观察到的 $\Delta U$ 值建立直方图。这个计算可以很容易地附加到标准MC移动中（算法 2），因为在试验移动中，我们随机选择一个粒子并计算其在试验移动前的相互作用能。而这个相互作用能正是我们想要计算的 $\Delta U$。由此我们获得 $\Delta U$ 的概率分布：$p_1(\Delta U)$。
    ...
    在图 8.4中，我们展示了如何从 $f_0(\Delta U)$ 和 $f_1(\Delta U)$ 关于 $\Delta U$ 的图中通过式（8.6.8）获得 $\mu^{\text{ex}}(\Delta U)$。左图的结果对应于 $\rho = 0.7$ 的Lennard-Jones流体，右图的结果对应于 $\rho = 1.0$。
    ...
    *图 8.4　重叠分布函数法与Widom粒子插入方案在测量Lennard-Jones流体化学势（$T = 1.2$）时的比较。实线为粒子插入结果，虚线为重叠分布法（$\beta \mu^{\text{ex}} = f_1 - f_0$）的结果。$\beta \mu^{\text{ex}}$ 的单位与 $f(\Delta U)$ 相同。左图对应于中等密度的液体（$\rho = 0.7$）。在此情况下，两个分布重叠，两种方法给出一致的结果。右图对应于高密度液体（$\rho = 1.00$）。在此情况下，插入概率非常低。分布 $f_0$ 和 $f_1$ 几乎不重叠，$\beta \mu^{\text{ex}}$ 的两种不同估计不一致。*
...
第8.6.1节中讨论的重叠分布方法首次展示了直方图如何在自由能计算中发挥作用。我们现在考虑更一般的情况：我们希望计算自由能剖面，例如在估算系统从初始状态（通常称为“反应物”态）到终态（“产物”态）所需越过的能垒时。自由能剖面依赖于坐标$Q$，它度量从初始态到终态的进展。$Q$通常被称为反应坐标或序参数，是系统微观（例如原子）坐标$X$的函数：
...
*图 8.5　含有一个无限薄、不可穿透的屏障的系统，该屏障相对于“序参数”$Q$倾斜放置。在这个系统中，作为$Q$的函数的自由能是平坦的，但从A到B并不存在物理路径。*

上面的例子说明，只有在指定了序参数$Q'$之后，我们才能定义自由能。自由能剖面还存在另一个问题：两点A和B之间的自由能剖面$F(Q)$相对平坦（即没有自由能势垒）并不一定意味着A和B之间存在物理路径。为了说明这一点，考虑一个简单的例子：矩形盒子中单个粒子的二维系统。盒子被一个无限薄但无限高的能量势垒斜切（见图8.5）。我们现在选择$x$坐标作为序参数$Q$。自由能通过在恒定$Q$下对$y$积分玻尔兹曼因子来计算。玻尔兹曼因子在恒定$Q$线上某一点为零这一事实对积分值没有影响。因此，$F(Q)$是常数。从A（左侧）到B（右侧）的路径看起来没有势垒。然而，显然从A到B不存在物理路径。上面的例子再次说明，谈论“自由能面”是没有意义的。识别好的序参数显然很重要。我们将在讨论稀有事件时回到这个问题（第15章）。
...
类似地，在恒压模拟中施加与体积$V$成正比的偏倚等价于在不同压力下采样系统，而偏倚$N_i$等价于在巨正则模拟中改变物种$i$的化学势。我们将在扩展系综采样方案的背景下回到这些热力学偏倚方法（参见第13.1.2节）。
...
一种广泛使用的获取态密度的蒙特卡罗方案由Wang和Landau引入（见第8.6.8节）。随后，我们将讨论重建任意自由能面的更近期技术。然而，出于教学原因，我们首先讨论偏倚采样的最早系统实例，即Torrie和Valleau^[347]的所谓伞形采样方法。
...
对“伞”权重函数 $\pi(X)$ 作恰当的选择时，式 (8.6.36) 的分子与分母都应当不为零。注意 $\pi(X)$ 不必具有 $\exp\{-\beta[U_0(X) + V^{\text{bias}}(X)]\}$ 的形式。事实上，$\pi(X)$ 在系统 0 与系统 1 的 Boltzmann 权重都为零的那部分构型空间中也可以不为零：这虽无益处，却也不致命。然而，在任一 Boltzmann 权重不为零的地方，$\pi(X)$ 都绝不应为零。

对于旨在计算体系在很宽的密度与温度范围内性质的计算，构造 $\pi(X)$ 的方法在文献 ^[229,348,349] 中有讨论。Berg 与 Neuhaus ^[350] 引入了伞形采样方法的一个变体，它在能量上施加偏倚，用以精确估计体系的态密度。文献 ^[350] 的方法被称为“多正则系综”方法（见补充材料第 L.5 节）。
...
???+ example "例证 10（正丁烷的伞形采样）"
    ...
    找到角度取特定值 $\varphi$ 的概率密度 $P(\varphi)$ 由下式给出{ 注：为方便起见，我们此处忽略了从笛卡儿坐标变换到广义坐标所伴随的 Jacobi 行列式（见第 14 章）。}：
...
以相同概率访问每一个状态、而不管其能量，是行不通的：这样的方案等价于无穷高温下的 Boltzmann 采样，模拟会把大部分时间花在采样势能极高的状态上，而那里往往是多个粒子相互重叠的非物理区域。我们几乎采样不到真正感兴趣的能量区间。

显然，我们希望有一种算法能够均匀地采样所有能量区间，而不是均匀地采样所有状态。这正是 Wang-Landau 类算法 ^[351,352] 所实现的：它逐步压低对应高态密度的那些能量的采样权重。

下面我们讨论态密度采样的基本原理。更多细节请参阅关于该方法的众多优秀综述（例如 ^[281,353]）。

不过在继续之前，我们要提醒读者：体系的（势）能 $U$ 只是序参数的一个例子。因此，尽管 Wang-Landau 方法是在计算“作为 $U$ 之函数的态密度”这一背景下发展起来的，其背后的思想可以（并且已经）被修改和推广，用于采样与其他序参数 $Q$ 相联系的自由能剖面。正是在这一背景下，我们随后将讨论 Calvo ^[354] 对 Wang-Landau 方法的推广，以及 Laio 与 Parrinello 的元动力学方法 ^[355]（见第 8.6.9 节）。

### Wang-Landau 采样

鉴于前面的讨论，Wang-Landau（WL）提出的采样态密度 $N(U)$ 的方法不基于对 Boltzmann 权重的重要性采样，也就不足为奇了。相反，该算法使用一种在模拟过程中不断改变所采样状态权重的 Monte Carlo 程序。其结果是，WL 算法并不满足细致平衡。然而，随着模拟收敛，细致平衡会被恢复 ^[351,356,357]。
...
下面我们假定初始时取 $g(U)$ 为常数。在模拟过程中，$g(U)$ 会在 $U$ 的取值范围内变化许多个数量级。因此，记录 $\ln g(U)$ 更为合适，初始取 $\ln g_0(U)=0$。只要 $g(U)$ 在 $U$ 的整个关心区间内非零，初始选择就无关紧要（不过参见文献 ^[358]）。
...
$f$ 的典型初值是 $e^1$。由于我们压低了态密度高的 $U$ 区域中构型的权重，采样将开始遍历整个关心能量区间的 $U$ 值，$H(U)$ 会变得更平坦，尽管仍相当嘈杂。事实上 $H(U)$ 之所以嘈杂有两个原因：其一，直方图某个箱中点数的（Poisson）涨落量级为 $\sqrt{m}$，$m$ 为该箱中的期望点数。增加用于构建直方图的样本数可以缓解（但无法消除）这一问题。其二，$H(U)$ 之所以嘈杂，是因为我们一直以固定步长更新 $\ln g(U)$：因此 $g(U)$ 永远不可能是光滑的 ^[359]。这一问题在算法的下一步中得到处理：我们把所有 $U$ 的 $H$ 重置为零，并减小 $\ln f$ 的值，例如

$$
\ln f \to \tfrac{1}{2}\ln f .
$$

注意我们并不重置 $\ln g(U)$。相反，在算法的下一轮中，我们继续细化 $\ln g(U)$，同时计算相应的 $H(U)$，后者应当变得越来越平坦。这一“减小 $\ln f$ 并重置 $H$”的过程一直持续，直到 $H$ 不再变得更平坦为止。该过程之所以会在某个细化程度上停止，是因为 $H(U)$ 始终保留着 Poisson 噪声，而这就限定了我们能把 $g(U)$ 细化到什么程度。当然，我们可以通过在直方图中收集更多计数来降低这一噪声。

关于 WL 算法收敛性质的详细讨论见文献 ^[359,360]。Zhou 与 Bhatt 还论证说，在每一个 Monte Carlo 步都更新态密度，会因相继更新之间的关联而导致系统误差。因此，确保两次更新之间的 Monte Carlo 步数足够大是重要的 ^[359]。
...
Wang-Landau 方法的一个明显前提是，我们必须充分采样所有相关构型。如果体系被阻止访问指定范围内的某些能量值，该方法就会失效。此时需要特殊技术才能恢复平坦的直方图 ^[358]。

另一个重要的注意事项是：至少对于能量在上方无界的体系（即普通的非格点体系——大多数自旋体系不属此列），我们需要给能量设定一个合理的上限。如果不设这个上限，全部模拟时间都会耗费在采样非物理的高能态密度上。

当我们考虑把 Wang-Landau 方法推广到能量以外的序参数时 ^[354]，最后这一点变得尤为重要。原因在于，正如前面所论证的，Wang-Landau 方法可以被视为一种无穷高温方法：态密度的采样并不带 Boltzmann 权重。然而，如果我们考虑能量以外的其他序参数，就必须确保体系仍然只探测物理上有意义的能量。正因如此，WL Monte Carlo 方法向其他序参数的推广 ^[354]，以及下面将讨论的元动力学 MD 方法，全都建立在偏倚 Boltzmann 采样之上。因此这类模拟的输出不是态密度，而是自由能剖面。

#### Wang-Landau 方法向其他序参数的推广

Calvo ^[354] 把 Wang-Landau 方法推广到对任意序参数 $Q = Q(X)$ 的 Boltzmann 加权概率分布进行采样。与前面一样，目标是在序参数的整个相关范围内探索概率分布，而不只是 $P(Q)$ 恰好较大的区域。秉承 Wang-Landau 方法的精神，这是通过逐步建立起一个偏倚、使 Monte Carlo 采样给出序参数的平坦分布来实现的。
...
最初的 Wang-Landau（WL）方法关注的是具有离散状态的自旋体系。但在讨论 Calvo 的方法时，我们已经隐含地假定 WL 方法可以推广到具有连续自由度的体系。例如，如果态密度是能量的连续函数，我们可以通过对能量分箱来估计该函数 ^[356,361]。更精确的做法是把分箱与插值方案结合起来 ^[356,362]。另一种做法是用一组在模拟过程中不断更新的高斯函数来表示态密度 ^[363]（这一做法与元动力学相似，见第 8.6.9 节）。

到目前为止我们讨论的都是一维直方图，但 Wang-Landau 方法可以推广到多维态密度。对于液体的模拟，这一点特别有用 ^[356,364]。例如，我们可以考虑处于恒定 $V$ 和 $T$ 的体系，并计算态密度 $N_{VT}(E,N)$，其中 $N$ 是粒子数，$E$ 是（势）能。一旦知道 $N_{VT}(E,N)$，我们就可以对任意 $\beta$ 与 $\mu$ 计算巨正则配分函数 $\Xi(\mu,V,T)$（见式 (2.3.19)）：
...
得到。这一方法可以推广到其他系综。例如，我们可以固定体系的压力和温度，把态密度确定为 $V$ 与 $E$ 的函数；或者固定化学势和温度，得到作为 $N$ 与 $E$ 之函数的态密度 ^[356,364]。

应用的例子包括对简单流体相行为的研究，例如确定 Lennard-Jones 体系的气-液 ^[356,364] 乃至液-固 ^[365] 共存曲线；该方法也已被用于更复杂的问题，例如蛋白质折叠 ^[366]。

### 元动力学

在上一节中，我们讨论了 Wang-Landau 方法以及旨在重建模型态密度（作为 $U$、$V$ 或 $N$ 之函数）的相关 Monte Carlo 方案，更一般地，还讨论了计算自由能剖面或自由能景观的方法 ^[354]。不出所料，这些方法已被推广到分子动力学模拟。不过，由此产生的 Laio 与 Parrinello 的“元动力学”方法 ^[355] 所做的，不止是把 Wang-Landau 搬到 MD 上。

在最基本的层面上，元动力学与 Calvo 的方法 ^[354]（见第 8.6.8 节）相似：它通过惩罚低自由能状态，迫使多体体系去采样自由能越来越高的状态。Huber 等人 1994 年的一篇论文 ^[367] 提供了这一思路在 MD 中的早期范例。
...
在继续之前我们要指出，在文献 ^[355] 的原始工作之后，元动力学得到了许多改进。改进之多，以至于目前元动力学已有各种品牌和风味，事实上与文献 ^[355] 的版本已相当不同，而后者本身又是受 Kevrekides 及其合作者的工作 ^[368] 启发的。特别地，元动力学起初是一种使用辅助变量和扩展 Lagrange 量的技术，其精神与 Andersen ^[180] 以及 Parrinello 和 Rahman ^[178] 的恒压、恒应力 MD 方法相同。然而，目前最流行的元动力学版本并不使用扩展 Lagrange 量方法。不过 Maragliano 与 Vanden-Eijnden 的温度加速动力学方法 ^[369] 确实使用了扩展 Lagrange 量方法。

下面我们不打算综述那些使元动力学变得更好的诸多精巧技术，而是把重点放在使元动力学与众不同的那些方面。关于实现的更多技术细节以及应用实例，我们请读者参阅该主题的一些优秀综述 ^[282,370,371]。

元动力学发展过程中的一项重要创新，是well-tempered 元动力学（Well-Tempered Metadynamics, WTM）的引入 ^[372]。它改进了方法的收敛速度，而且重要的是，它的收敛性确实已被证明 ^[373]——在文献 ^[373] 的工作之前，这一点一直被普遍假定却未获证明。

WTM 通过改变在自由能景观不同部位所沉积高斯的幅度（在 $V_b(q;t)$ 已经很大的 $q$ 处沉积得更慢）来改进向平坦直方图的收敛。具体做法 ^[372] 是把在时刻 $t$、点 $q$ 处沉积高斯的速率由 $w_G$ 改为 $w_G\exp(-\alpha V_b(q;t))$，其中 $\alpha$ 为常数。不过这一做法会影响渐近平坦直方图 $P_g(q)$（记号与上一节相同）与底层自由能剖面 $F(q)$ 之间的关系。这里我们用一个比文献 ^[372] 更为启发式的论证来得到这一结果。注意 $V_b(q;t)$ 增长的速率正比于体系出现在 $q$ 处的概率，即 $P_g(q)$ 乘以 $q$ 处的沉积速率：
...
用文献 ^[372] 的记号（其中 $\alpha \equiv 1/\Delta T$，$\beta = 1/T$），并忽略与 $q$ 无关的偏移 $c'(t)$，我们得到 $F(q) = [T/(T+\Delta T)]V_b(q;t)$。因此在 well-tempered 情形下，$F(q) \neq -V_b(q)$。

well-tempered 元动力学的一个优点是，它能逐步降低自由能估计中的噪声。关于元动力学方法的这些以及其他方面，我们请读者参阅相关文献 ^[372--374]。关于元动力学与平行回火等扩展系综技术相结合的例子，请参阅文献 ^[375--377]。
...
假设我们希望在区间 $Q_{\max} - Q_{\min} \equiv \Delta Q$ 上计算自由能剖面 $F(Q)$。我们将比较两种做法所需的估计模拟时间：在整个 $\Delta Q$ 范围上做一次自由能计算，与在宽度为 $\Delta Q/n$ 的较小窗口上做 $n$ 次这样的计算。这类多窗口伞形采样方法的一个例子，是 Berg 与 Neuhaus 的多正则采样方案 ^[350]。

$n$ 的最优选择显然应当是能在最短机时内采样完整个 $Q$ 区间的那个。为估计这一时间，我们假定体系在窗口 $\Delta Q/n$ 内于 $Q$ 空间中作随机游走。扩散的假设是合理的，因为一旦我们用 WL 或元动力学把自由能剖面压平，体系就只是在区间 $\Delta Q/n$ 内扩散。与 $Q$ 空间中的随机游走相联系的是扩散常数 $D_Q$，为方便起见我们假定它是常数；扩散系数变化的更一般情形见 ^[378]。
...
注意估计的机时随 $n$ 增大而减小。然而，若据此认为 $n$ 应取得越大越好，那就错了。某个 $Q$ 窗口中一次运行的实际平衡化时间，还取决于所有与 $Q$ 正交的坐标被采样的速率。把这一时间记为 $\tau_\perp$。显然，一旦 $\tau_\perp$ 明显大于 $\tau_n$，总计算量将按 $n\times\tau_\perp$ 标度。这提示 $n$ 的最优选择是使 $\tau_n \approx \tau_\perp$ 的那个。更详细的讨论见 ^[58]。我们在第 13 章还会回到多窗口自由能计算。

把自由能剖面拆分成多个窗口可以加快模拟（例如见 ^[379--381]），但这是有代价的：自由能剖面与概率分布的对数相联系，而这些分布现在是按每个窗口分别归一化的。结果是自由能曲线的不同部分相互之间发生了平移；为重建完整曲线，我们必须把不同窗口中的曲线“缝合”起来。

人们提出了许多把局部自由能剖面合并成单条曲线的技术，但在撰写本版时，其中一种技术——多态 Bennett 接受比方法（Multistate Bennett Acceptance Ratio, MBAR）^[382]——已因若干原因而占据主导地位。首先，许多早期方法用直方图表示概率分布。使用直方图的问题在于必须选择分箱宽度：选得太宽，所有细节都会丢失；选得太窄，许多箱会是空的，而如果我们想用 $\beta F(Q) = -\ln P(Q)$，空箱就成了问题。相比之下，MBAR 是无分箱的。MBAR 的第二个优点是它不使用插值、外推或拟合：MBAR 使用全部采样数据点，而且只使用数据点。最后也是最重要的优点是，MBAR 在一个明确定义的意义上是最优的。下面我们描述 MBAR 方法。
...
当然，像上面那样由 $\delta$ 函数之和构成的分布并不适合作图表示。事实上，它看起来一点也不像我们习惯的直方图。为看清这一点，考虑在一次无偏倚模拟中生成的分布函数。把 $V_k = 0$ 代入最大似然条件，可以看到对所有 $n$ 都有 $p_n = 1$。那么，一个由等幅 $\delta$ 函数之和构成的分布函数，怎么会代表 Boltzmann 分布呢？

答案是：在 Boltzmann 权重高的区域，$\delta$ 函数的数目远多于权重低的区域。因此，如果我们把上述分布函数在宽度为 $\Delta$ 的箱上积分来构造直方图，就会重新得到一个看起来确实像 Boltzmann 分布的直方图。重要的是，用 MBAR 我们只需一次性确定最优分布，之后再做分箱或曲线拟合即可。
...
因此，通过测量序参数分布中相邻窄窗口之间的跃迁速率，我们就可以重建 $P(Q)$ 的形状 ^[383--386]。

## 非平衡自由能方法

上面我们讨论了若干计算自由能差的技术。所有这些技术要么假定所研究的体系处于热力学平衡，要么（如元动力学）假定体系随时间缓慢变化。这种对处于平衡态或接近平衡态之体系的选择看似合乎逻辑，因为两个状态之间的自由能差等于把一个状态变为另一个状态所需的可逆功。因此，令人惊讶的是：两个体系之间的自由能差竟然也可以通过计算把一个体系变为另一个体系所需的非平衡功而得到。事实上，我们将讨论的关系式对任意短的“切换”时间 $t_s$ 都成立。下面我们简要介绍 Jarzynski ^[387,388] 的非平衡自由能表达式，以及 Crooks ^[389--391] 提出的一些推广。

与前面一样，我们考虑两个 $N$ 粒子体系：一个具有 Hamilton 量 $\mathcal{H}_0(\Gamma)$，另一个具有 $\mathcal{H}_1(\Gamma)$，其中 $\Gamma \equiv \{\mathbf{p}^N,\mathbf{r}^N\}$ 表示体系的相空间坐标。我们假定可以把 $N$ 粒子体系的 Hamilton 量从 $\mathcal{H}_0$ 切换到 $\mathcal{H}_1$——也就是说，引入一个依赖于含时切换参数 $\lambda(t)$ 的 Hamilton 量 $\mathcal{H}_\lambda$，使得 $\lambda=0$ 时 $\mathcal{H}_{\lambda=0} = \mathcal{H}_0$，$\lambda=1$ 时 $\mathcal{H}_{\lambda=1} = \mathcal{H}_1$。于是我们可以写出
...
注意，由于 Hamilton 量的切换而对体系做的功 $W$ 等于 $\int_0^{t_s}\mathrm{d}t\,\dot{\lambda}\,\partial\mathcal{H}_\lambda[\Gamma(t)]/\partial\lambda$。如果切换进行得非常缓慢，体系在整个变换过程中保持平衡，$W$ 便退化为把体系 0 变为体系 1 所需的可逆功。在这种条件下，$W(t_s\to\infty) = F_1 - F_0 \equiv \Delta F$。然而对有限的切换时间，把体系由状态 0 变到状态 1 所需消耗的平均功 $\overline{W}$ 大于自由能差 $\Delta F$：

$$
\overline{W}(t_s) \geq \Delta F .
$$

功 $W(t_s)$ 依赖于穿过相空间的路径；对 Hamilton 体系而言，这条路径本身又依赖于初始相空间坐标 $\Gamma(0)$（稍后我们会考虑有许多条路径连接 $\Gamma(0)$ 与 $\Gamma(t_s)$ 的更一般情形）。接下来考虑 $\exp[-\beta W(t_s)]$ 的平均。功 $W(t_s)$ 是初始相空间位置 $\Gamma(0)$ 的函数。我们假定在 $t=0$ 时体系处于热平衡，此时找到体系 0 处于相空间位置 $\Gamma(0)$ 的概率由正则分布给出：
...
这里我们用到了 $W(t_s) = \mathcal{H}_1[\Gamma(t_s)] - \mathcal{H}_0[\Gamma(0)]$。最后，我们利用 Hamilton 运动方程保面积这一事实，它意味着 $\mathrm{d}\Gamma(t_s) = \mathrm{d}\Gamma(0)$。于是得到 Jarzynski 的核心结果：
...
这就是式 (8.6.10)。Crooks ^[389] 给出了不局限于 Hamilton 体系的、更一般的推导。特别地，Crooks 证明了只要体系的动力学是 Markov 的且微观可逆，上述结果仍然成立。Crooks 的结果意味着，即使体系的“时间演化”由 Metropolis Monte Carlo 方案决定，该结果依然有效。更多细节见附录 E。

Jarzynski 的结果及 Crooks 的推广既令人惊讶又优雅，具有重大的概念意义。然而，几乎没有证据表明计算自由能差的非平衡方法优于现有的平衡方法 ^[392]。

例外似乎是玻璃态体系中自由能差的计算 ^[393]——在那里热力学积分及相关方法干脆失效。然而，当其他自由能方法也可以使用时，它们至少与 Jarzynski 方法一样好。限制 Jarzynski 方法实际应用的根本原因在于：对于远离平衡的变换，上述关系式的统计精度可能相当差。我们对瞬时切换的极限早已知道这一点。就像 Widom 方法的粒子移除版本一样，我们希望计算的平均值的主要贡献来自很少被采样到的初始构型。这正是在测量化学势时，“粒子移除”法无法成为“粒子插入”方案之可行替代的原因。为了在非平衡自由能计算的背景下说明这一问题，我们考虑一种不改变体系自由能的 Hamilton 量变化。

一个例子是把液体中一个分子平移距离 $+X$ 的 Monte Carlo 移动。如果 $X$ 与典型分子尺度相比不算小，那么该粒子的位移多半需要做正功。把粒子从新位置沿 $-X$ 移回起点的反向情形亦然。然而初态与末态的自由能相同，因此 $\Delta F$ 应为零。这意味着：在所有构型中，功为负的那极小一部分构型，对 $\exp(-\beta W)$ 的平均作出了大小相等、符号相反的贡献。事实上，正如粒子插入／粒子移除的情形一样，问题的化解之道在于把正向与反向方案结合起来。下面我们以 Hamilton 体系为例加以说明，但结果是一般的。

现在我们考虑两个非平衡过程：一个在时间间隔 $t_s$ 内把 Hamilton 量从 $\mathcal{H}_0$ 变为 $\mathcal{H}_1$，另一个做相反的变换。对这两个过程，我们都可以把变换过程中所消耗的功作成直方图。对正向过程，可以写出
...
上述结果似乎把 Jarzynski 结果的适用性限制在正向功与反向功分布有不可忽略重叠的情形，而这通常只在接近平衡时才成立。然而，正如 Hartmann ^[394] 所指出的，通过对非平衡轨迹作偏倚路径采样，该方法也可以在极端非平衡的情形下奏效。此外，Nilmeier 等人 ^[395,396] 证明了文献 ^[389] 的方法可以用来构造复合 MC 试探移动，使其在普通 MC 移动接受率很低的条件下仍有很高的接受率。
...
**问题24（虚拟体积变化）。**正如第 5.1.5.3 节所讨论的，维里方程对计算硬球流体的压力并不特别方便。
...
[^6]: 如果固体包含大量空位，Gibbs 系综方法似乎仍然可能有效。然而，在这种情况下会出现另一个问题：在含有大量空位的固体中，粒子数可能与晶格位点数有很大差异。如果忽略这种效应，模拟仍然会给出答案，但将是错误的（参见^[224]）。
