...
当然，像式 (8.6.49) 那样由 $\delta$ 函数之和构成的分布并不适合作图表示。事实上，它看起来一点也不像我们习惯的直方图。为看清这一点，考虑在一次无偏倚模拟中生成的分布函数。把 $V_k = 0$ 代入最大似然条件，可以看到对所有 $n$ 都有 $p_n = 1$。那么，一个由等幅 $\delta$ 函数之和构成的分布函数，怎么会代表玻尔兹曼分布呢？
