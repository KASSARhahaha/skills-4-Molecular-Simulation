...
在第2.5节中，我们讨论了单组分系统的结构因子$S(\mathbf{q})$（式(2.5.15)）与粒子密度傅里叶变换的均方值（式(2.5.16)）之间的关系：
...
其中$\delta\rho_i(\mathbf{q})$表示$\rho(\mathbf{q})$围绕其平均值的涨落。如果我们在式的后半部分取$q \to 0$的极限，可得：

$$
\lim_{q\to 0} S_{ab}(q) = \sqrt{\frac{\langle \delta N_a\,\delta N_b \rangle}{\langle N_a \rangle \langle N_b \rangle}},
$$

其中$\delta N_a$表示系统中物种$a$的粒子总数的涨落。沿用Kirkwood和Buff的方法^[717]，我们将证明式具有直接的热力学解释，这为确定溶液中化学势随组成变化的依赖关系提供了一条强有力的途径。
...
将式与式进行比较，可以看出结构因子$S_{ab}(\mathbf{q})$在$q \to 0$极限下的行为与巨势的热力学导数之间存在密切关系。

Kirkwood和Buff首先提出了这些关系^[717]。然而，他们也将结果表示为对径向分布函数$g_{ab}(r)$的积分形式，许多模拟研究使用$g(r)$方法来计算化学势随组成的变化。基于$g(r)$的关系在原理上是正确的，但正如第2.5节下方所解释的，在（甚至不非常小的）小系统模拟中使用它们是非常危险的。因此，最好坚持使用式^[805]。[^1]

我们还需要解释为什么式中的关系很重要。关键原因是，在粒子插入方法（见）失效的条件下，它们使我们能够计算多组分混合物中各物种化学势随组成的变化。
...
由Gibbs-Duhem方程（$\mathrm{d}\Omega|_{V,T} = -\sum_r \langle N_r \rangle\,\mathrm{d}\mu_r$）可得：
...
其中$\delta_{rs}$是Kronecker $\delta$函数。换言之，一旦我们计算出了粒子数涨落的交叉关联矩阵，我们就知道了所有物种化学势随组成的变化规律。需要注意的是，$\mathbf{A}$矩阵对应于$T$、$V$和$N'$固定的系综，而$\mathbf{B}$则表达了恒定$\mu$、$V$、$T$系综中的涨落。

## 模拟中的Kirkwood-Buff方法

为了计算混合物的相行为，我们需要知道化学势随组成的变化。对于稠密液体，我们不能使用粒子插入方法（）或巨正则模拟（第6.3节）来计算$\mu_i$，因为成功插入的概率变得非常小。正是在这样的条件下，式变得有用：如果不同物种的粒子数是固定的，$S_{ab}(\mathbf{q}=0)$恒等于零，但我们可以计算$q \to 0$时的$S_{ab}(\mathbf{q}=0)$极限，该极限定义良好，且除可能的有限尺寸效应外，等于所需的值。

---

[^1]: 文献^[806]描述了在各物种分子总数固定的系统中利用Kirkwood-Buff关系的另一种方法。