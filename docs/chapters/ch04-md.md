...
第 5.1.1 节将说明，温度通常由每个自由度的平均动能来测量。但这一描述对于 MD 模拟来说是一种过度简化，因为在MD中我们必然使用离散时间步长。主要原因是，正如文献^[114,115]所指出的，由速度Verlet算法生成的速度与动量之间的关系并不简单地是$v_i = p_i/m_i$。相反，“真实”速度通过哈密顿运动方程定义：$\dot{r}_i^{\mathrm{true}} \equiv \partial H/\partial p_i$。对于使用有限时间步长的模拟，哈密顿量并不守恒，而是“影子”哈密顿量$H_S$守恒（见式(4.3.22)）。正是这个哈密顿量决定了相空间中的密度，并应该出现在玻尔兹曼因子中。我们可以使用
...
*图 4.2　使用NVT分子动力学模拟计算的蛋白质（实线）和溶剂（虚线）的温度随时间的变化。上图使用不正确的温度表达式(4.3.10)，下图使用正确的表达式(4.3.8)。使用不正确的表达式时，蛋白质和溶剂之间存在明显的表观温度差异。*
