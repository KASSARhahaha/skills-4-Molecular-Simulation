...
为了简化这个搜索，我们引入一个在 RG 算法中起重要作用的概念：我们将区分“开放”的试探方向和“关闭”的试探方向。要确定单体$j$的给定试探方向$b$是否开放，我们计算其能量$u_j(b)$。试探位置$b$开放的概率[^9] 由下式给出：
...
[^9]: 这个概率还可以有许多别的取法，并可用来优化模拟。不过这里讨论的这一种取法，对 Lennard-Jones 势和硬核势似乎都很有效。
[^10]: 最大试探取向数应事先选定——并且可以依赖于指标 $i$——但在其他方面是任意的。