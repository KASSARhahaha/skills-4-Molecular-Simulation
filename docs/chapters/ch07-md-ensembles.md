...
牛顿动力学与随机碰撞的混合将分子动力学模拟转变为一个随机马尔可夫过程^[67]。正如^[180]中所证明的，相空间中的正则分布在随机碰撞的重复应用下是不变的，加上马尔可夫链也是不可约的和非周期的^[180,254,255]，这意味着Andersen算法确实产生正则分布。在算法 16和 17中，我们展示了如何在分子动力学模拟中实现Andersen方法。
...
1. 出于算法 16 之后所解释的原因，**integrate-A** 使用速度-Verlet 算法^[113]。
...
???+ example "例 14（Andersen 恒温器的使用）"
    ...
    Andersen恒温器^[180]可以说是最简单的已被证明能产生正则分布的MD恒温器。这意味着所考虑系统的动能和势能都是玻尔兹曼分布的。
...
???+ example "例证 7（受限气体的扩散系数）"
    ...
    对于某些系统，人们希望模拟少得多的粒子。一个例子是多孔材料中扩散系数的计算。假设你有兴趣与吸附在这种多孔介质中的分子的实验扩散系数进行比较。在这种情况下，如果我们将计算的扩散系数外推到零负载极限^[193]，这种比较会变得更容易。因此，模拟少量气体分子具有直接相关性。对于这样小的数量，$3N$和$3N - 3$之间的差异可能很显著。在图 7.2中，Xu等人^[107]比较了使用正确和不正确自由度数的Nos\'{e}-Hoover模拟计算的扩散系数。在Nos\'{e}-Hoover哈密顿量中，配分函数由（见公式(7.1.16)）给出。

    ![图 7.2](../images/fig_7_2.png)

    *图 7.2　CH$_4$ 在金属有机骨架 M2(dobdc)（M：Mg、Ni 和 Zn）中的自扩散系数随负载量的变化。图中比较了使用正确与不正确的自由度所得的结果。本图基于文献 ^[107,266] 的数据。*
...
???+ example "例 15（Nos\'{e）"
    ...
    *图 7.3　Lennard-Jones 流体中的速度分布（$T = 1.0$，$\rho = 0.75$，$N = 256$）。实线为 Maxwell-Boltzmann 分布 (7.1.1)。符号为使用 Nosé-Hoover 恒温器 ^[256] 的模拟结果。*
