...
式 (15.5.10) 表明，对于远大于$k_B T$的势垒，$\tau_\mathrm{MFPT}^{(A \to B)}$的值由势垒顶部附近$q$范围内的$D(q)$值主导。更一般的式 (15.5.5) 可用于计算 A 和 B 之间$q$值处的界面的 MFPT，$\tau_\mathrm{MFPT}^{(A \to q)}$随$q$的变化可用于重构自由能势垒[[660]](references.md#ref-660)。
