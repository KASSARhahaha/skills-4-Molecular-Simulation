...
了解Newton运动方程足以理解分子动力学方法的基础。然而，许多更高级的模拟技术利用了经典力学的Lagrange表述或Hamilton表述。在这里，我们简要勾勒这些不同方法之间的关系（另见）。关于经典力学更详细和更严格的描述，读者可参考Goldstein的著作。
...
$$
m\mathrm{d}ot{x} = -\frac{\partial U(x)}{\partial x},
$$
...
$$
\mathrm{d}ot{\theta} = -\frac{1}{ml^2}\frac{\partial U(\theta)}{\partial \theta}.
$$
...
如果我们定义式(A.4.9)中定义的$\mathbf{M}$的转置矩阵[^4]$的转置矩阵可以通过交换行和列得到，即$\tilde{a}_{ij} = a_{ji}$。}：
...
这个条件通常被称为辛条件。满足此条件的矩阵$\mathbf{M}$称为辛矩阵[^5]}$的逆矩阵：
...
[^4]: 给定矩阵$\mathbf{A
[^5]: 要看出此条件与式(A.4.6)和(A.4.7)等价，我们必须从右边将此方程乘以$\tilde{\mathbf{M