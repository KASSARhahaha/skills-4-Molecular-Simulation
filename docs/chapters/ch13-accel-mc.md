...
[^3]: 译注：下式照录原书。按式 (13.4.2) 并结合边界条件 $q = 0 \leftrightarrow q' = 0$ 积分，可得 $q = (\beta\kappa C)^{-1}\left[1 - \mathrm{e}^{-\beta\kappa q'}\right]$；再由 $q$ 在 $[0,1]$ 上均匀分布定出 $C = (\beta\kappa)^{-1}$，于是正确的结果应为 $q' = -(\beta\kappa)^{-1}\ln(1 - q)$。
